...
每天定时为 **WPS灵犀 / MiniMax Code / 百度搭子 / Qoder / 魔搭 / WorkBuddy / TraeWork**
自动签到，
...
docker compose exec ai-checkin python -m app.main status   # 各平台凭证与今日状态
...
| WorkBuddy | 不支持网页登录 | `scan` 自动读 `%LOCALAPPDATA%\CodeBuddyExtension\...\workbuddy-desktop.info`（需桌面端已登录） |
| TraeWork | 不支持网页登录 | `scan` 自动读 wb-switch 捕获的桌面端登录态（`agent_accounts.json` 的 trae 条目） |
...
python -m pytest            # 277 项，全离线（本地回环假服务器 + 真实报文黄金样本）
```
