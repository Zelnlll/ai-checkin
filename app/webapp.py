...
def _cred_fingerprint(acct: dict[str, Any]) -> tuple[list[str], str]:
    """([凭证哈希...], JWT uid)：token/cookie 各算一份，供推送工具身份
    路由；不回显凭证本身。"""
    hashes = []
    for k in ('token', 'cookie'):
        prim = str(acct.get(k) or '')
        if prim:
            hashes.append(hashlib.sha1(prim.encode('utf-8')).hexdigest()[:8])
    uid = ''
    tok = str(acct.get('token') or '')
    if tok.count('.') >= 2:
        try:
            import base64
            seg = tok.split('.')[1]
        ...
        items.append({
            ...
            'accounts': [dict(
                {'id': aid, 'label': label,
                 ...
                 'hashes': _cred_fingerprint(acct)[0],
