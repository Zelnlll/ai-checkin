...
- `/api/status` 每账号暴露 `hash`（凭证 sha1 前 8 位）与 `uid`（JWT sub），
  不回显凭证本身。
- PC 工具据此三路判定：哈希同=原位刷新 / uid 同=换票刷新 / uid 新=自动
  追加副账号（客户端切号后双击即收）；`--login wps,dazi,…` 推送前静默
  刷新浏览器 Cookie（storage_state 有效不弹窗）。

## v1.5.4 — 2026-09-23

### 新功能：推送工具多账号 + 防裂号
- PC 工具改推 wb-switch 全部账号（第一个=主账号 id=main），并按 JWT
  uid 归并同账号多票（取到期最远）；面板 upsert 新增"同名 label 唯一
  匹配"通道：副账号 token 轮换原位更新，不再裂第三号。
- 双击 `推送凭证.bat` 免命令行运行。

## v1.5.3 — 2026-09-23

### 优化
- 面板鉴权文案由"访问令牌"改为"访问密码"。
...
### 修复：合并单容器后二轮评审
- 凭证/状态加进程内互斥锁（多线程共享 pid，文件锁超时兜底时读改写
  不再互踩）；tmp 文件名追加线程 id；clear() 纳入锁内（并发"删不掉复活"）。
- PANEL_PORT 非法值不再 ValueError 崩守护，回落 8000。
- 面板一键签到加同平台防重入锁（连点不再并发打官方接口，忙时 409）。
- compose 回滚注释版本号修正；.env.example 补 PANEL_PORT。
- 评审确认：锁无重入死锁；守护主循环不受面板请求阻塞。
...
### 优化：积分明细弹窗重做
- 多账号不再纵向堆到末尾：顶部账号标签页（mtab）切换，一屏只看一个账号。
- 长包名单行省略号截断，悬停/点按 title 看全称；行栅格名称列允许收缩。

## v1.4.6 — 2026-09-23

### 优化
- 通知第二行缩进定稿（用户选 h 版）：全角+两半角空格，行2 首字对齐
  行1 平台名首字。

## v1.4.5 — 2026-09-23

### 优化
- 通知第二行定稿列序：`余X｜连N天｜到期积分`（原到期在最前）。
